# -*- coding: utf-8 -*-
# ---------------------------------------------------------------------------
# Post-process Phenology Senescence
# Author: Timm Nawrocki
# Last Updated: 2022-12-27
# Usage: Must be executed in an ArcGIS Pro Python 3.7 installation.
# Description: "Post-process Phenology Senescence" calculates mean senescence date and corrects to predicted surface types.
# ---------------------------------------------------------------------------
...
input_folder = os.path.join(project_folder, 'Data_Output/output_rasters', round_date, 'phen_senescence')
output_folder = os.path.join(project_folder, 'Data_Output/data_package', version_number, 'phen_senescence')
...
for input_set in input_list:
    ...
    for year in input_set:
        input_raster = os.path.join(input_folder, f'GMT2_Phen_Senescence_{str(year+2000)}.tif')
        input_rasters.append(input_raster)

    # Define output raster
    if count == 1:
        output_raster = os.path.join(output_folder, 'GMT2_Phen_Senescence_2005.tif')
    elif count == 2:
        output_raster = os.path.join(output_folder, 'GMT2_Phen_Senescence_2010.tif')
    elif count == 3:
        output_raster = os.path.join(output_folder, 'GMT2_Phen_Senescence_2015.tif')
    else:
        output_raster = os.path.join(output_folder, 'GMT2_Phen_Senescence_2020.tif')
