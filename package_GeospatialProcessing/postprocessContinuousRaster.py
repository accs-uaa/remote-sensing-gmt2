...
def postprocess_continuous_raster(**kwargs):
    ...
    Inputs: 'calculate_mean' -- either True or False
            ...
            'input_array' -- an array containing the area raster (must be first), the categorical raster (must be second), the infrastructure raster (must be third), and the raster or rasters (if calculate_mean is True) to post-process
    ...
    infrastructure_raster = kwargs['input_array'].pop(0)
    ...
    # Set values to 0 for infrastructure and conditional statement
    ...
    # Set values to 0 where infrastructure exists
    print('\t\tRemoving infrastructure...')
    raster_remove_1 = Con(Raster(infrastructure_raster) == 1, 0, mean_raster)
    ...
    arcpy.management.CopyRaster(raster_remove_2,
                                output_raster,
                                '',
                                '',
                                no_data_value,
